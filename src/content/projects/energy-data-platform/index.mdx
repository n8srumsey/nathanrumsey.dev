...
annotations:
  - term: "medallion architecture"
    detail: "A layered data pattern with Bronze (raw ingestion), Silver (cleaned and standardized), and Gold (analytical/aggregated) layers. Bronze preserves source fidelity, and Gold is the data product accessible to downstream data warehouses."
    style: tech
  - term: "Delta tables"
  ...
  - term: "minimal-IAM storage integration"
    detail: "An AWS IAM policy granting Snowflake read-only access to exactly one S3 prefix with no write permissions nor cross-account access. Implements least-privilege to keep the ETL and warehouse layers fully decoupled."
    style: tech
---
